...
There are currently three types of CENM database schemas:
...
The services **must** use separate database schemas (either in the same database instance or in completely separate instances) due to the way the migrations are defined. If you try and run an Identity Manager, Network Map, or Zone Service that shares the same database schema, it will result in errors.

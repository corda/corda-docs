...
In a disaster recovery scenario, if you have been unable to use the automated LedgerRecover process to recover lost data, you can use the Manual LedgerRecover approach instead.

LedgerRecover (Manual)[workflows](#workflowsection) are  manually initiated and executed by the respective node operators of the nodes
involved.

As these processes are managed on a discretionary basis, it isn't necessary to enforce the same restrictions on the size, duration or other
criteria used with an automatic recovery process.

However, you should be extremely careful when accepting and executing manually initiated `RecoveryRequests`, even more so when the requested
...
In this context, _self-consistent_ means that all transactions and dependencies are correctly recorded in the vault and all appropriate tables. _Current_, refers to whether or not the node which experienced a disaster has a record of all transactions to which they should have been informed occurred on the network.
...
The initiating node then persists a `RecoveryRequest` - which is a record containing the details of the recovery. The initial record is persisted in a `REQUESTED` status indicating it has not been acted on by the counterparty. The initiating node will then request that the counterparty also persist a `RecoveryRequest`.
...
* `RecoveryRequest` - This flow will return the entire `RecoveryRequest` object that has been successfully persisted by the initiating node. This will be displayed in the console as follows:
...
* `failReason` - A message indicating the reason we are failing this `RecoveryRequest`.

...
* Optionally, a web server `.jar` file with a name `corda-webserver-4.5.jar`, which will connect to the node via RPC. Some alternatives to this are listed below:

    * You can use the default web server **for testing**. Note that this feature is deprecated - do not use it in a production environment.
...
task deployNodes(type: net.corda.plugins.Cordform, dependsOn: ['jar']) {
    ...
    node {
        ...
        // No webport property, so no web server will be created.
        h2Port   10004
...
* `name` &lt;string&gt; - use this configuration option to specify the legal identity name of the Corda node. For more information, see [myLegalName](../setup/corda-configuration-file.md#corda-configuration-file-mylegalname). For example:

```kotlin
name "O=PartyA,L=London,C=GB"
```
* `p2pAddress` &lt;string&gt; - use this configuration option to specify the address/port the node uses for inbound communication from other nodes. For more information, see [p2pAddress](../setup/corda-configuration-file.md#corda-configuration-file-p2paddress). **Required if `p2pPort` is not specified**. For example:

```kotlin
p2pAddress "example.com:10002"
```
* `p2pPort` &lt;integer&gt; - use this configuration option to specify the port the node uses for inbound communication from other nodes. The assumed IP address is `localhost`. For more information, see [p2pAddress](../setup/corda-configuration-file.md#corda-configuration-file-p2paddress). For example:

```kotlin
p2pPort 10006  // "localhost:10006"
```

* `rpcSettings` &lt;config&gt; - use this configuration option to specify RPC settings for the node. For more information, see [rpcSettings](../setup/corda-configuration-file.md#corda-configuration-file-rpc-settings). For example:
...
* `webAddress` &lt;string&gt; - use this option to configure a web server to connect to the node via RPC and to specify the address and port the node listens on. The node must have an RPC address configured. For more information, see [Specifying a custom web server](#specify-custom-webserver). For example:

```kotlin
webAddress  "example.com:10011"
```

* `webPort` &lt;integer&gt; - use this option to configure a web server to connect to the node via RPC. The default address is `localhost`. The node must have an RPC address configured. For more information, see [Specifying a custom web server](#specify-custom-webserver). For example:

```kotlin
webPort  10011  // "localhost:10011"
```

* `rpcUsers` &lt;list&gt; - use this configuration option to set the RPC users for the node. For more information, see [rpcUsers](../setup/corda-configuration-file.md#corda-configuration-file-rpc-users). You can use arbitrary values in this configuration block - "incorrect" settings will not cause a DSL error. An example follows below:
...
* `https` &lt;boolean&gt; - use this configuration option to enable (set to `true`) or disable (set to `false`) HTTPS communication from the node's web server. For example:

```kotlin
https true
```

* `sshdPort` &lt;integer&gt; - use this configuration option to specify the port for sshd communication. For more information, ee [sshd](../setup/corda-configuration-file.md#corda-configuration-file-sshd). For example:
